...
export async function POST(request: Request, ctx: RouteContext<"/api/zones/[id]/launches">) {
  ...
  // Доли оплаты у «По факту» на старте невозможны: цена ещё не известна, её
  // считает остановка пуска (launches/[id]/stop зовёт validateSplitLegs со
  // своей суммой). Проверка обязана стоять ЗДЕСЬ, вне ветки «За вход», —
  // генеральная проверка финансов 2026-09-02 нашла, что validateSplitLegs
  // живёт внутри блока `pricingMode === "fixed"`, а списание с кошелька ниже
  // (строка ~257) выполняется для любого режима. То есть у «По факту» доли
  // принимались вообще без проверки: одного запроса с отрицательной суммой
  // хватало, чтобы дорисовать клиенту баланс.
  //
  // Второй рубеж — assertSpendable в lib/abonement.ts: он ловит тот же случай
  // у любого будущего вызывающего. Здесь отказ раньше и с внятной причиной.
  if (legs && pricingMode !== "fixed") {
    return NextResponse.json(
      { error: "Оплата долями возможна только у тарифа «За вход»: у «По факту» сумма известна при остановке" },
      { status: 400 }
    );
  }
