...
/**
 * Реальные ли это деньги бизнеса — про аванс и премию.
 *
 * ПРАВИЛО ВЛАДЕЛЬЦА 2026-09-04: «аванс или премия, которую вносит владелец,
 * отражается только на сотруднике, но в реальных деньгах нигде не
 * учитывается». Он даёт их из своего кармана: касса точки не худеет, расхода
 * у бизнеса не возникает, меняется только личный баланс сотрудника
 * «к выдаче» (он держится на beneficiaryOperatorId и этой проверки не знает).
 *
 * Признак — performedByOperatorId, «из чьих рук ушли деньги», ровно тот же,
 * по которому getPointCashBalance решает, вычитать ли выплату из ящика:
 *
 *   только performedByUserId          — владелец из кармана: не деньги бизнеса
 *   performedByOperatorId             — сотрудник взял сам из кассы: деньги
 *   оба поля                          — владелец дописал в смену то, что
 *                                       сотрудник реально взял из кассы: деньги
 *
 * Одна проверка на все экраны: Главная, Деньги и Отчёты → Динамика считали
 * Прибыль каждый по-своему, и расхождение между ними уже было предметом
 * правки 2026-07-25 — теперь они спрашивают об этом в одном месте.
 */
export function payoutIsBusinessMoney(op: { performedByOperatorId: string | null }): boolean {
  return op.performedByOperatorId !== null;
}
...
// собственные пулы ниже — свип вычитается из них ПО СУММЕ, а не сдвигает
// отсечку по дате (getPointAbonementCashTotal / getPointGoodsCashTotal).
...
// по абсолютно всем наличным деньгам на точке") — свой пул, независимый от
// товарного: свип вычитается из него по сумме, отсечки по дате тут нет
// (её убрали 2026-09-02 — частичный свип отсекал и то, что не забирали). Только
...
// у getPointAbonementCashTotal выше, но свой, независимый пул (свип
// вычитается по сумме) — нужен явной цифрой и для потолка "что честно
...
/**
 * Доли пула ОСТАЛЬНЫХ зон точки — списываются той же транзакцией, что и
 * инкассация одной зоны (генеральная проверка финансов, С21/С23).
 *
 * Дефицит раскладывается пропорционально по ВСЕМ зонам, но инкассация зоны
 * добирала только свою долю. После неё отсечка аванса сдвигалась, аванс
 * выпадал из остатка точки, и на следующем расчёте дефицит выходил нулевым —
 * доли остальных зон не списывались уже никогда:
 *
 *   A 600, B 400, аванс сотрудника −300 → дефицит 300, доли A 180 / B 120
 *   инкассация A на 600 пишет −780, остаток зон 220, отсечка сдвинута
 *   следующий расчёт: max(0, 220 − 220) = 0 → долг B в 120 повис навсегда
 *
 * Журнал точки при этом оставался завышенным против физических денег, а зона,
...
//     независимый пул ("2 пачки", запрос
