...
3. **Кнопка добавления** — пилл «+ Добавить» в шапке секции (`.btn-add`,
   тёмный фон `--ink`, не акцентный) **или** пунктирная строка `add-inline`
   внутри карточки («+ Устройство/ссылка активации», «+ Добавить актив»).
...
| `.btn-add` | `Button` с кастомным классом (тёмный `--ink` фон, не `--primary`) — уточнить, заводить ли `variant="dark"`/`variant="add"` в `buttonVariants`, при реализации |
