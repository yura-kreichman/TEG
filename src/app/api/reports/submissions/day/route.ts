  ...
  const cards = submissions.flatMap((s) =>
    s.zoneSubmissions.map((zs) => {
      ...
      return {
        ...
        // Расходы, которые сдача забрала себе — слагаемое Разницы. Отдаём
        // наружу, чтобы шторка правки считала ту же формулу, что и карточка
        // (генеральная проверка финансов 2026-09-02, С18): своего источника
        ...
        // Инкассация до пересчёта — ОТДЕЛЬНЫМ числом, помимо суммы выше
        // (вопрос владельца 2026-09-03: «в итогах дня 2945, хотя по факту
        // было больше — не учтена инкассация 5350»). Итог дня складывал
        // только сданное сотрудником, и экран занижал выручку ровно на эту
        // величину. Шторке правки по-прежнему нужна СУММА (она только
        // складывает), а карточке дня — слагаемое, чтобы его назвать.
        collectedBefore,
