...
// Куда вернуть после входа. Только собственный путь — иначе ссылка вида
// /login?next=https://… уводила бы человека, уже введшего пароль, на чужой
// сайт (классический открытый редирект, который любят в фишинге: адрес в
// письме ведёт на настоящий my.rentos365.app).
//
// Обратный слэш проверяется отдельно (аудит 2026-08-13): браузеры нормализуют
// "\" в "/" при разборе адреса, поэтому "/\evil.com" — это на самом деле
// "//evil.com", то есть переход на другой домен, а проверка startsWith("//")
// его не ловила. Отвергаем весь символ целиком, а не пытаемся угадать, где он
// безопасен: в легитимном пути приложения его не бывает.
