...
export function LandingJsonLd({ data, baseUrl }: { data: LandingRenderData; baseUrl: string }) {
  ...
  // Основной сайт компании (запрос владельца 2026-08-16) — именно в sameAs:
  // это "тот же бизнес по другому адресу". Organization.url остаётся
  // каноническим адресом ЭТОЙ страницы — подмена вернула бы ошибку, которую
  // чинил SEO-аудит 2026-07-14.
  if (data.websiteUrl && !sameAs.includes(data.websiteUrl)) {
    sameAs.push(data.websiteUrl);
  }
