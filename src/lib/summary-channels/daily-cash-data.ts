...
export async function buildDailyCashSummaryData(
  ...
  // НА КОНЕЦ ДНЯ сводки, а не «сейчас» (генеральная проверка финансов
  // 2026-09-02). Все остальные суммы здесь считаются по окну bounds, и только
  // остаток брался текущим: пересобранная сводка за вчера показывала
  // сегодняшний остаток кассы. Экран «Итоги дня» тот же вопрос задаёт с
  // отсечкой — теперь и сводка тоже, иначе два места про один день снова
  // отвечали бы по-разному.
  const cashOnHand = await getPointCashBalance(pointId, prisma, bounds.end);
