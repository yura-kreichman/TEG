...
export async function GET(request: Request) {
  ...
  // Границу бизнес-дня тенанта здесь сознательно НЕ передаём (переход кабинета
  // на бизнес-день, 2026-08-06): посещение сайта в два ночи — это посещение
  // именно этой календарной даты, а не "поздний вечер предыдущего рабочего
  // дня". Смена и касса — событие точки, у которого есть рабочий день;
  // посетитель лендинга к смене не привязан. LandingDailyStat.date и так
  // хранится календарным днём.
