...
export default function OperatorHomePage() {
  ...
  // Реальный баг, найден пользователем 2026-07-22: владелец снял у оператора
  // последнюю разрешённую зону на его текущей роуминг-точке — /api/operator/
  // points уже исключает точки без единой разрешённой зоны, так что
  // "тупиковая" текущая точка выпадает из списка целиком, остаётся ровно одна
  // (другая) валидная точка. Показывать в этом случае обычный переключатель
  // было только хуже: он выглядел как рабочий, но текущая точка (её и так
  // некуда выбрать — её просто нет среди опций) отображалась как будто бы
  // выбранная, вместе с пропавшей иконкой (лукап иконки по списку опций не
  // находит текущую точку — она в список не входит). Раз выбор всё равно
  // ровно один — просто переключаем автоматически, без лишнего шага и без
  // сбивающего с толку интерфейса.
  useEffect(() => {
    if (!roaming || !pointId || points.length !== 1) return;
    if (points[0].id === pointId) return;
    handleSwitchPoint(points[0].id);
  }, [roaming, points, pointId]);
