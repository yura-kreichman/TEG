...
$templates = [
    ...
    'ticket_created_email_to_customer' => [
        ...
        'body'    => '<p>Здравствуйте, <strong>{{customer.first_name|коллега}}</strong>!</p>'
            ...
            . '<p>Ответ придёт на этот адрес. Отвечать на письмо не нужно — чтобы что-то добавить, откройте заявку по ссылке выше.</p>'
    ...
    'ticket_replied_by_agent_email_to_customer' => [
        ...
        'body'    => '<p>Здравствуйте, <strong>{{customer.first_name|коллега}}</strong>!</p>'
            ...
            . '<h4><a href="{{ticket.public_url}}">Открыть заявку</a></h4>'
            . '<p>Чтобы ответить, откройте заявку по ссылке выше. Отвечать на само письмо не нужно — такой ответ в заявку не попадёт.</p>'
            . '<hr /><p>С уважением,<br />{{business.name}}</p>',
    ],
