...
/**
 * TranslatePress: переводить ещё два атрибута — адрес картинки в ленивой загрузке
 * и подпись лайтбокса.
 *
 * Зачем. На «Возможностях» две карусели скриншотов приложения, и для en/it/ro
 * подложены англоязычные снимки (пары заведены в словаре). Но карусель Elementor
 * с включённой ленивой загрузкой печатает адрес в `data-src`, а `src` не печатает
 * вовсе. TranslatePress `data-src` умеет, но только как побочный эффект перевода
 * `src` (см. translate_image_srcset_attributes) — без `src` подмена не
 * запускалась, и на английской странице слайды оставались русскими, тогда как
 * ссылка лайтбокса уже была английской. Разнобой был виден сразу.
 *
 * Второй атрибут — `data-elementor-lightbox-title`: подпись в увеличенном виде,
 * тот же текст, что в alt, и переводы для него в словаре уже есть.
 *
 * Состояние на 2026-08-09: ленивую загрузку в каруселях владелец попросил
 * выключить, и `data-src` там больше не печатается — работает первый обработчик
 * впустую, второй по-прежнему нужен (18 подписей на странице). Оставлен на случай
 * возврата ленивой загрузки: вреда нет, а грабли повторять не хочется.
 */
add_filter('trp_node_accessors', function ($accessors) {
    ...
    $accessors['image_data_src'] = [
        'selector'  => 'img[data-src]',
        'accessor'  => 'data-src',
        'attribute' => true,
    ];
    $accessors['elementor_lightbox_title'] = [
        'selector'  => '[data-elementor-lightbox-title]',
        'accessor'  => 'data-elementor-lightbox-title',
        'attribute' => true,
    ];
