...
export default async function manifest(): Promise<MetadataRoute.Manifest> {
  ...
  return {
    ...
    // scope ОБЯЗАТЕЛЕН при динамическом start_url (найдено 2026-08-13).
    // Без него спецификация выводит область из самого start_url, обрезая имя
    // файла: на привязанном устройстве start_url = "/operator/login", значит
    // областью становится "/operator/" — и весь кабинет владельца (/points,
    // /money, /reports, /login) оказывается ВНЕ приложения. Браузер такие
    // страницы не блокирует, но показывает на них собственную полосу с
    // адресом или уводит в отдельное окно; на это же похожа жалоба
    // 2026-08-13 «ярлык PWA открывает Chrome».
