...
export async function POST(request: Request, ctx: RouteContext<"/api/zones/[id]/tariffs">) {
  ...
  // Лимит "максимум 2" — у Счётчиков, аппаратно обоснован (запрос
  // пользователя 2026-07-27: "Счётчики — это аппаратные устройства... в
  // других режимах учёта их нет"). У остальных режимов лимита не было вовсе
  // (запрос того же дня), но тем же вечером решение пересмотрено ещё раз:
  // мягкий потолок "4" — не для аппаратных причин, а чтобы владелец не
  // захламлял зону тарифами (реальные варианты цены внутри одного тарифа
  // уже покрывают "будни/выходные" — TariffOption, не отдельные Tariff;
  // несколько ТАРИФОВ нужны только под разные по цене активы одной зоны,
  // такого редко бывает больше 3-4).
  const maxTariffs = zone.accountingMode === "counters" ? 2 : 4;
  if (activeTariffs.length >= maxTariffs) {
    return NextResponse.json(
      // Оба текущих значения (2/4) — "тарифа" в родительном падеже (2-4
      // одинаково склоняются в русском), менять число выше безопасно;
      // склонение сломается только если появится 1 или 5+, сюда не подходит.
      { error: `У зоны уже максимум ${maxTariffs} тарифа` },
      { status: 409 }
    );
  }
