      ...
      {justCredited ? (
        ...
        <>
          {!initialWallet && (
            ...
            {/* Продажа плана и произвольная сумма — ВНЕ обёртки
                {!initialWallet} выше (баг, найден пользователем 2026-08-14:
                "у владельца в Клиентах пропала возможность начислять
                произвольный баланс"). Обёртка прячет шаги поиска и создания,
                которые при заданном initialWallet не нужны, — но утянула за
                собой и эти два блока, а карточка клиента в кабинете
                открывается именно с initialWallet. В итоге под заголовком
                "Пополнить баланс" не рисовалось ничего. Ровно это и обещает
                комментарий у самого пропса: "остаётся только выбор
                плана/произвольной суммы".

                Продажа плана — только Сотрудник (запрос пользователя
                2026-07-18: "Продаёт только сотрудник"), у Владельца секция
                целиком скрыта пропсом allowPlanPurchase, а не этой обёрткой. */}
