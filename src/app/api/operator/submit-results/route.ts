...
export async function POST(request: Request) {
  ...
  async function runSubmission(tx: Prisma.TransactionClient) {
    // Лок ТОЧКИ — первым, до локов зон (закрывающий аудит 2026-09-03). Вся
    // денежная семья точки сериализуется по hashtext(pointId): инкассация
    // зоны (api/zones/[id]/collection), общая инкассация, доли пула,
    // карточные аванс/премия, chargeSelfServiceAdvanceToZones,
    // settleOutstandingCollectionAdvance. Сдача же лочилась только по зонам —
    // ключи разные, и инкассация зоны шла ПАРАЛЛЕЛЬНО со сдачей той же зоны.
    // Пока сдача журнал зоны не читала, это было терпимо. Теперь читает:
    // getZoneCollectionOverdraw ниже возвращает деньги, которые владелец
    // забрал из ящика ДО пересчёта, и подмешивает их в выручку. Инкассация,
    // проскочившая в зазор (закоммитилась после того, как сдача сняла свой
    // снимок), не компенсируется уже никогда: окно следующей сдачи начинается
    // с createdAt этой, а операция с меньшим occurredAt уходит в «дефицит на
    // начало окна» и из прироста вычитается — это и есть минус из воздуха
    // (жалоба КидсБурга 2026-09-02, которую чинил сам overdraw).
    // Порядок «сначала точка, потом зона» — общий по проекту; обратного
    // порядка захвата в коде нет (hashtext(zoneId) встречается только здесь),
    // так что deadlock этот лок не добавляет.
  ...
  // сводках наравне с "Прибываниями" (см. summary-channels/*-format.ts).
    ...
    // выручку строкой выше, и складывать оба слагаемых значило считать его
    // дважды:
    //
    //   касса 0 (вчера всё забрали), выручка дня 1000, расход 350
    //   сотрудник вводит 650; дефицит 350 и компенсация 350
    //   было:  650 + 350 + 350 − 1000 = +350 излишка из воздуха,
    //          в журнал уходило revenue 1350 при 650 в ящике
    //   стало: 650 + 350 +   0 − 1000 = 0
    //
    // Вычитаем, а не выбрасываем расходы из самого прохода: когда расход
    // случился ДО инкассации среди дня, компенсация его не берёт (деньги на
    ...
    for (const zs of zoneSubmissions) {
      ...
      const zoneSubmission = await tx.zoneSubmission.create({
        data: {
          ...
          cashAmount: zs.cashAmount,
          mobileAmount: zs.mobileAmount,
          // Сколько расходов эта сдача вернула в выручку (С4). Ровно та же
          // величина, что прибавлена к revenue строкой ниже, — сохраняем её,
          // потому что из журнала потом не восстановить, какие именно расходы
          // были компенсированы: «привязанные» и «компенсируемые» намеренно
