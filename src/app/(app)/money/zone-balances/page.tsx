...
export default function ZoneBalancesPage() {
  ...
  // Сворачивать ли акт в строку «Инкассация» с раскрытием по тапу.
  //
  // Раньше условием было просто «в акте больше одной записи», и от этого одна
  // и та же по смыслу операция выглядела по-разному (замечание владельца
  // 2026-09-04): инкассация, задевшая зону и кассу Товаров, давала две записи
  // и сворачивалась, а инкассация одной зоны рисовалась плоской строкой с
  // НАЗВАНИЕМ ЗОНЫ вместо слова «Инкассация» — по экрану было не понять, что
  // это то же действие. Теперь инкассация выглядит одинаково всегда.
  //
  // Акты НЕ склеиваются между собой (решение владельца 2026-09-04: «отдельными
  // строками, у каждой же своё время») — в отличие от авансовой инкассации,
  // которую склеивают по дню, потому что там владелец жмёт кнопку на каждую
  // зону отдельно в рамках одного захода.
  //
  // Одиночные строки, которые сворачивать НЕЛЬЗЯ: размен (деньги в кассу, у
  // него своя подпись и знак) и аванс/премия, забранные сотрудником
  // самостоятельно, — у последних вся строка ведёт в карточку сотрудника, и
  // подменять эту ссылку раскрытием значило бы сломать правку записи.
  // Формально они попадают в kind "collection" только потому, что у них нет
  // actKey.
  function isCollapsibleAct(act: CollectionAct): boolean {
    if (act.items.length > 1) return true;
    if (act.kind !== "collection") return false;
    const pool = act.items[0]!.pool;
    return pool !== "advance_taken" && pool !== "bonus_taken" && pool !== "change_fund";
  }
