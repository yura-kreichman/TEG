...
`DELETE /api/admin/tenants/[id]/route.ts` — безвозвратное удаление тенанта и АБСОЛЮТНО всех его данных (задокументировано здесь впервые, аудит 2026-07-27 — сама возможность существовала раньше без единой строчки в спеке). Механизм — не прикладной код, а `onDelete: Cascade` на каждой relation в `prisma/schema.prisma`, ведущей к `Tenant` (прямо или транзитивно через Point/Zone/Operator/User и т.д.) — один `DELETE FROM "Tenant" WHERE id = ...` каскадно сносит всё на уровне самого Postgres.

Найдено и исправлено дважды (миграции `20260723200828_fix_tenant_delete_cascade_fks` и `20260726215605_fix_remaining_tenant_delete_cascade_fks` + `20260726220340_fix_operator_created_by_cascade`): обязательная (не `?`) relation без явного `onDelete: Cascade` в Prisma по умолчанию получает `Restrict`, а не `Cascade` — если целевая модель тоже удаляется независимой каскадной цепочкой от `Tenant`, порядок срабатывания двух независимых каскадов Postgres не гарантирует, и `DELETE Tenant` мог упасть FK-ошибкой на любом тенанте, где эта связь хоть раз использовалась. Аудит 2026-07-27 прошёл по всей схеме систематически и не нашёл больше необработанных мест — при добавлении НОВОЙ модели с обязательной relation к чему-то, что каскадно живёт под тенантом, `onDelete: Cascade` нужно проставлять явно, дефолт Prisma здесь не то, что нужно.

Осознанное исключение: `LandingVisitorSeen.landingId` — вообще без `@relation`/FK (суточный дедуп-хэш посетителей лендинга, самоочищается плановой очисткой старых дней независимо от жизни тенанта) — не блокирует `DELETE`, просто не имеет смысла как настоящий FK.

### Удаление (2026-09-19): одного каскада мало

Решение владельца: удаление — **полное**. Клиенты и абонементы, бот перестаёт работать для этого владельца, медиафайлы удаляются, ничего связанного с ним не остаётся. Всё делает одна функция `deleteTenantEverywhere` (`src/lib/tenant-lifecycle.ts`) — и для ручного удаления, и для массовой чистки, и для автоудаления Free-кабинетов.

Каскад Cascade не спасает строки, у которых **кроме каскадной связи есть связи SET NULL** (необязательная relation без `onDelete` в Prisma получает именно SET NULL). Postgres удаляет родителя (зону, точку) раньше самой строки, а обнуление другой её связи перепроверяет уже несуществующего родителя — FK-ошибка. Так 2026-09-19 не удалялся ни один тенант с пусками (park). Поэтому такие таблицы удаляются явно, в той же транзакции и до тенанта, пока живы их родители: `AbonementTransaction`, `CounterTapEvent`, `Launch`, `TicketOrder`, `GoodsSale`, `GoodsHeldOrder`, `GoodsReconciliation`, `GoodsRevision`, `MoneyOperation`, `Asset`, `Landing` (их дочерние — оплаты частями, билеты — уходят каскадом). **Новая таблица с каскадной связью и хотя бы одной SET NULL-связью должна попасть в этот список.** Найти все такие — запросом к `pg_constraint` по `confdeltype` ('c' и 'n' у одной таблицы).

Что каскад не видит и функция убирает сама:
- `CorrectionLog` по данным тенанта (`entityId` — голая строка, а в before/after лежат суммы, балансы и имена клиентов) и сделанный его людьми, включая записи о самом тенанте;
- `LandingVisitorSeen` его лендинга, `WebhookEvent` с его `tenantId`;
- недоделанные шаги клиентского бота (`ClientBotSession.pendingTenantId` / `pendingRegistrationTenantId`) — обнуляются, сессия чата общая для всех тенантов.

После удаления в базе:
- бот выходит (`leaveChat`) из клиентской группы и Telegram-каналов сводок тенанта — кроме чатов, которыми пользуется другой тенант или группа уведомлений платформы;
- стирается папка `public/uploads/<tenantId>/`. Приложение в контейнере должно иметь права на неё: оно работает под `SITE_UID`/`SITE_GID` из `.env` прода, и это **uid владельца папки сайта** (md33 = 1010), потому что хук выкладки делает `chown -R md33:md33` на всё дерево при каждом деплое. С uid 501 загрузки и удаление файлов молча ломались после каждой выкладки со 2 по 19 сентября;
- удаляется учётная запись на маркетинговом сайте.
...
- **Владелец может запретить вход техподдержки** (решение владельца 2026-08-23, конфиденциальность данных кабинета): тумблер "Доступ техподдержки" в Настройках владельца под карточкой плана, поле `Tenant.supportAccessEnabled`, по умолчанию доступ ОТКРЫТ. При выключенном тумблере кнопка "Impersonate" в карточке тенанта неактивна, попытка запроса отдаёт 403, а УЖЕ НАЧАТАЯ имперсонация обрывается на ближайшем же запросе (проверка в `requireOwner`), не дожидаясь истечения сессии. Из имперсонированной сессии сам тумблер не переключается — снять запрет может только владелец. Остальной админ-модуль (статус, лимиты, биллинг) запретом не затрагивается
