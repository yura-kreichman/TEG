...
export interface ZoneSummaryData {
  ...
  // 2026-09-03). Одной строкой «Ушло» это смешивало траты сотрудника и
  // инкассацию владельца — владельцу читалось как выдуманный термин. Оба
  // слагаемых входят в «Разницу», и без них она не сходится глазами:
  // «Виртуалка: касса 800, счётчики 1250, Разница −100. С чего?»
  // Показываются только когда не ноль.
...
export interface DailyCashSummaryData {
  ...
  expenses: number;
  // Сколько владелец забрал инкассацией ДО пересчёта кассы (вопрос владельца
  // 2026-09-03: «по факту наличных было больше, тут не учтена инкассация
  // 5350 — картина итогов дня неполная»).
  //
  // cashAmount выше — это то, что сотрудник ФИЗИЧЕСКИ СДАЛ, и его смысл не
  // меняется: слово «Наличные» уже переопределяли 2026-08-16, и это
  // породило целый класс багов. Вместо переопределения — отдельная строка,
  // а «Итого» ниже складывает всё:
  //
  //   КидсБург, 02.09: сдано 2945, забрано днём 5350
  //   было:  «Наличные 2945» — и ни слова о том, где ещё 5350
  //   стало: «Наличные 2945 · Инкассация 5350», Итого 8295 + безнал
  //
  // Это та же дыра, что закрыта утром в сводке ЗОНЫ, только на уровень
  // выше: там сводка Машинок честно писала «Инкасс. 3000», а итог дня по
  // той же зоне давал 2425 — сводки спорили друг с другом.
  collectedDuringDay: number;
