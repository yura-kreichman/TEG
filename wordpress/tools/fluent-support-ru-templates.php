...
// Тема писем клиенту намеренно оставлена как есть — 'Re: {{ticket.title}} #{{ticket.id}}'.
...
$subjectToCustomer = 'Re: {{ticket.title}} #{{ticket.id}}';
...
$templates = [
    ...
    'ticket_created_email_to_customer' => [
        ...
        'body'    => '<p>Здравствуйте, <strong>{{customer.first_name|коллега}}</strong>!</p>'
            . '<p>Мы получили вашу заявку «<strong>{{ticket.title}}</strong>» (<a href="{{ticket.public_url}}">#{{ticket.id}}</a>) — она уже у службы поддержки.</p>'
    ...
    'ticket_replied_by_agent_email_to_customer' => [
        ...
        'subject' => $subjectToCustomer,
        'body'    => '<p>Здравствуйте, <strong>{{customer.first_name|коллега}}</strong>!</p>'
            . '<p>По вашей заявке «<strong>{{ticket.title}}</strong>» (<a href="{{ticket.public_url}}">#{{ticket.id}}</a>) пришёл ответ:</p>'
    ...
    'ticket_closed_by_agent_email_to_customer' => [
        ...
        'subject' => $subjectToCustomer,
        'body'    => '<p>Здравствуйте, <strong>{{customer.first_name|коллега}}</strong>!</p>'
            . '<p>Ваша заявка «<strong>{{ticket.title}}</strong>» (<a href="{{ticket.public_url}}">#{{ticket.id}}</a>) закрыта.</p>'
    ...
    'ticket_created_by_agent_email_to_customer' => [
        ...
        'subject' => $subjectToCustomer,
        'body'    => '<p>Здравствуйте, <strong>{{customer.first_name|коллега}}</strong>!</p>'
            . '<p>Служба поддержки завела заявку «<strong>{{ticket.title}}</strong>» (<a href="{{ticket.public_url}}">#{{ticket.id}}</a>) по вашему вопросу.</p>'
            . '<h4><a href="{{ticket.public_url}}">Открыть заявку</a></h4>'
            . '<hr /><p>С уважением,<br />{{business.name}}</p>',
    ],
    ...
    'ticket_created_by_agent_on_behalf_email_to_customer' => [
        'status'  => 'yes',
        'subject' => 'Заявка создана (#{{ticket.id}})',
        'body'    => '<p>Здравствуйте, <strong>{{customer.first_name|коллега}}</strong>!</p>'
            . '<p>Служба поддержки завела для вас заявку «<strong>{{ticket.title}}</strong>» (#{{ticket.id}}):</p>'
            . '<div style="' . $quote . '">{{response.full_content}}</div>'
            . '<h4><a href="{{ticket.public_url}}">Открыть заявку</a></h4>'
            . '<hr /><p>С уважением,<br />{{business.name}}</p>',
    ],
    ...
    'ticket_created_email_to_admin' => [
        'status'  => 'yes',
        'subject' => 'Новая заявка: {{ticket.title}} #{{ticket.id}}',
    ...
    'ticket_replied_by_customer_email_to_admin' => [
        'status'  => 'yes',
        'subject' => 'Новый ответ: {{ticket.title}} #{{ticket.id}}',
    ...
    'ticket_agent_on_change' => [
        'status'  => 'yes',
        'subject' => 'Заявка назначена вам: {{ticket.title}} #{{ticket.id}}',
        'body'    => '<p>Здравствуйте, <strong>{{agent.full_name}}</strong>!</p>'
            . '<p>На вас назначена заявка <a href="{{ticket.admin_url}}">«{{ticket.title}}» #{{ticket.id}}</a>.</p>',
    ],
];
