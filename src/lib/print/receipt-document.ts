...
function receiptCss(paperWidth: ReceiptPaperWidth): string {
  ...
  * { box-sizing: border-box; }
  ...
  /* filter ВРЕМЕННО убран (диагностика 2026-07-28, MHT-P5801) — живой тест
     подтвердил: печать без лого проходит нормально, файл лого маленький
     (640×221px, 16KB — не в размере исходника дело). Следующий шаг —
     проверить, тяжела ли для растеризации именно CSS-фильтрация
     (grayscale+contrast+brightness — честный per-pixel проход) сама по
     себе, при том же изображении. Если это тоже зависает — дело не в
     фильтре, а в самом факте растрового <img> в документе на этом мосту, и
     фильтр можно будет вернуть как есть. Прежний смысл фильтра (см. историю
     git) — убрать цвет и поднять контраст ДО дизеринга драйвером принтера,
     а не полагаться на дизеринг сырого цвета. */
  .receipt-logo {
    max-width: 180px;
    max-height: 90px;
    margin: 0 auto 5px;
