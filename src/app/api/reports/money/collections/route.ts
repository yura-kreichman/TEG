...
export async function GET(request: Request) {
  ...
  // Ключ одного АКТА инкассации: исполнитель, точка и время строки.
  //
  // ВНИМАНИЕ, здесь долго стояла НЕВЕРНАЯ посылка. Было написано, что все
  // строки одной транзакции получают идентичный occurredAt, потому что он
  // объявлен DEFAULT CURRENT_TIMESTAMP, а это время начала транзакции, — и
  // что группировка по метке поэтому не эвристика, а следствие способа
  // записи. На проде метку ставит Prisma каждой строке отдельно, и они
  // расходятся на миллисекунды (КидсБург 2026-09-04: .390 / .412 / .415 у
  // трёх зон одной инкассации). Склейка по точному совпадению не срабатывала
  // никогда — общая инкассация всегда рассыпалась по зонам.
  //
  // Поэтому время в ключе оставлено (оно нужно, чтобы разные заходы не
  // слиплись), но СРАВНИВАЕТ его клиент с допуском в 2 секунды — см.
  // splitIntoActs в money/zone-balances. Здесь ключ хранит время как есть, а
  // всё после первого «|» — это устойчивая часть, исполнитель и точка.
