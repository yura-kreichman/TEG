...
export async function GET(request: Request, ctx: RouteContext<"/api/points/[id]/reports/zones">) {
  ...
  if (drillZone) {
    ...
    tariffBreakdown = drillZone.tariffs
      // Удалённый тариф показываем, ТОЛЬКО если он что-то заработал за период
      // (замечание владельца 2026-09-03: «показывается тариф Вторая скорость,
      // которого нет — когда-то был тестовым давно»). Удалённые тарифы
      // подтягиваются сюда нарочно, иначе выручка тарифа, стёртого после
      // сдачи, молча пропадала бы из разбивки. Но тестовый тариф, не
      // заработавший ни рубля за всю историю, висел строкой «0 ₽ · 0%»
      // навсегда. Живые тарифы остаются даже с нулём: «по этому тарифу за
      // период не продали ничего» — это осмысленный ответ, а не мусор.
      .filter((t) => t.deletedAt === null || (perTariffRaw.get(t.id) ?? 0) !== 0)
      .map((t) => {
